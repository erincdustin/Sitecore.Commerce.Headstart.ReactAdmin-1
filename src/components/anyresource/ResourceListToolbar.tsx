...
interface ResourceListToolbarProps extends Omit<ListViewChildrenProps, "renderContent"> {}

const ResourceListToolbar: FC<ResourceListToolbarProps> = ({
  ...
  return (
    <>
      <Stack direction="row" mb={5}>
        <DebouncedSearchInput label="Search {placeholder}" value={queryParams["Search"]} onSearch={updateQuery("s")} />
        <ProductFilters />
        ...
        {viewModeToggle}
        <Button variant="primaryButton">Create Item</Button>
      </Stack>
      ...
      <Modal isOpen={isOpenMassEditProducts} onClose={onCloseMassEditProducts}>
        ...
        <ModalContent>
          {isMassEditing ? (
            ...
            <>
              <ModalHeader>Mass Edit Items</ModalHeader>
              <ModalCloseButton />
              <ModalBody pb={6}>
                <Text>You have selected {selected.length} items</Text>
                <FormControl mt={4}>
